...
export function dmgKeyboard(page: number) {
  const start = page * 10;
  const end = start + 9;
...
export function monsterDmgKeyboard(page: number) {
  const start = page * 10;
  const end = start + 9;
  const nums = Array.from({ length: 9 }, (_, i) => start + i).filter(
    (n) => n <= 100,
  ...
  rows.push(
    nums
      .slice(0, 3)
      .map((n) => Markup.button.callback(`${n}⚔️`, `BATTLE_MONSTER_DMG_${n}`)),
  );
  // 4 5 6
  rows.push(
    nums
  ...
  if (page > 0) arrowRow.push(Markup.button.callback("◀️", "DMG_LEFT"));
  arrowRow.push(
    Markup.button.callback(`${last}⚔️`, `BATTLE_MONSTER_DMG_${last}`),
  );
  if (page < 9) arrowRow.push(Markup.button.callback("▶️", "DMG_RIGHT"));
